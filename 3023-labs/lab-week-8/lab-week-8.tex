...
plot(sig.sq, power.rej, type = "l", main = "Power of two tail test", xlab = "sigma squared", ylab = "power")
...
alpha = 0.04
n = 5
uniroot(f = fn, lower = 0, upper = qchisq(alpha, df = n - 1) - eps, alpha = alpha, n = n)
\end{Sinput}
\begin{Soutput}
$root
[1] 0.5395026
...
norm.var.umpu = function(alpha, n) {
    ...
    eps = 1e-08
    # calculate c
    c = uniroot(f = fn, lower = 0, upper = qchisq(alpha, df = n - 1) - eps, alpha = alpha, n = n)$root
    # calculate d. Recall: P0(Y <= d) = 1 - alpha + P0(Y <= c) [since Y is continuous]
    d = qchisq(1 - alpha + pchisq(c, df = n - 1), df = n - 1)
    return(list(c = c, d = d))
}
...
# for the UMPU test: 1. find the values of c and d according to alpha = 0.04 and n = 5
...
# legend
legend("top", legend = c("Power of equal-tailed test", "power of UMPU test"), col = c("black", "blue"),
    lty = c(1, 1))
\end{Sinput}
...
norm.var.glrt = function(alpha, n) {
    ...
    eps = 1e-08
    # calculate c
    c = uniroot(f = fn.glrt, lower = 0, upper = qchisq(alpha, df = n - 1) - eps, alpha = alpha, n = n)$root
    # calculate d. Recall: P0(Y <= d) = 1 - alpha + P0(Y <= c) [since Y is continuous]
    d = qchisq(1 - alpha + pchisq(c, df = n - 1), df = n - 1)
    return(list(c = c, d = d))
}
